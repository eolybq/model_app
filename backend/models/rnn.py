...
def train_model(data, seq_len, predicted_feature, batch_size, epochs):
    ...
    model.add(keras.layers.LSTM(512, activation='tanh', input_shape=(x_train.shape[1], x_train.shape[2]), return_sequences=True))
    model.add(keras.layers.Dropout(0.2))
    model.add(keras.layers.LSTM(512, activation='tanh'))
    model.add(keras.layers.Dropout(0.2))
    ...
    checkpoint = keras.callbacks.ModelCheckpoint(filepath='./rnn/big_model_checkpoint.keras', 
                            save_best_only=True, 
                            monitor='val_loss', 
                            mode='min')
...
def predict(new_data, seq_len, number_features, predicted_feature):
